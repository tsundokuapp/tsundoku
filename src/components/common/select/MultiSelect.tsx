...
const multiSelectVariants = cva(
  'm-1 transition-all duration-300 ease-in-out rounded-sm',
  {
    variants: {
      variant: {
        default: 'text-black bg-appSearchText hover:bg-appSearchText/80',
        secondary:
...
export const MultiSelect = React.forwardRef<MultiSelectRef, MultiSelectProps>(
  (
    ...
    const hoverDelete =
      'transition-all duration-300 hover:scale-125 hover:text-red-500';
      ...
      <>
        ...
        >
          <div id={triggerDescriptionId} className="sr-only">
            Multi-select dropdown. Use as setas para navegar, Enter para
            selecionar e Esc para fechar.
          </div>
          <div id={selectedCountId} className="sr-only" aria-live="polite">
            {selectedValues.length === 0
              ? 'Sem itens selecionados'
              : `${selectedValues.length} ite${
                  selectedValues.length === 1 ? 'm' : 'ns'
          ...
          <PopoverTrigger asChild>
            <Button
              ...
              ia-controls={isPopoverOpen ? listboxId : undefined}
              aria-describedby={`${triggerDescriptionId} ${selectedCountId}`}
              aria-label={`Multi-select: ${selectedValues.length} de ${
                getAllOptions().length
              } itens selecionados. ${placeholder}`}
              className={cn(
                'flex h-auto min-h-10 items-center justify-between rounded-md bg-appInputBackground p-1 hover:bg-appInputBackground [&_svg]:pointer-events-auto',
              ...
              {selectedValues.length > 0 ? (
                <div className="flex w-full items-center justify-between">
                        ...
                        return (
                          <Badge
                            ...
                            className={cn(
                              ...
                              customStyle?.gradient && 'text-white',
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    >
                      <XIcon className="h-4 w-4 transition-all duration-300 hover:scale-125 hover:text-red-500" />
                    </div>
                    <Separator
            ...
            <Command>
              ...
              {searchable && (
                <div id={`${multiSelectId}-search-help`} className="sr-only">
                  Digite para filtrar opções. Use as teclas de seta para navegar
                  pelos resultados.
                </div>
              )}
              <CommandList
