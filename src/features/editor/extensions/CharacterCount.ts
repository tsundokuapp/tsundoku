...
export interface CharacterCountOptions {
  /**
   * The maximum number of characters that should be allowed. Defaults to `null` (no limit).
   * @default null
   * @example 180
   */
 ...
 * Custom implementation based on @tiptap/extension-character-count
 */
...
>({
  ...
  onBeforeCreate() {
    ...
    this.storage.words = (options?: { node?: ProseMirrorNode }) => {
      const node = options?.node || this.editor.state.doc;
